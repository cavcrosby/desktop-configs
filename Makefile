...
>	@printf '%s\n' '                                       in gsettings.txt'
>	@printf '%s\n' '  ${ADD_APT_SOURCES}                    - add APT data sources/sources.list(5) files and those related'
...
${SERVE_PRESEED_CONFIG}: override export ENCRYPTED_PASSWORD := $(if ${ENCRYPTED_PASSWORD},$(value ENCRYPTED_PASSWORD),$$6$$496388125$$lkA4dPUIN/ueRva.PE3bfRTsq6DRKUw7Ir02VygOih9ufg2.SI.4c9L2xOEh4qgGojD3x1NttO8Fu6WygqxII/)
${SERVE_PRESEED_CONFIG}: override export ENCRYPTION_PASSPHRASE := $(if ${ENCRYPTION_PASSPHRASE},$(value ENCRYPTION_PASSPHRASE),Passw0rd!)
